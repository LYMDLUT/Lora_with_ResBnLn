...
class LoraBlock(nn.Module):
    def __init__(self, host:Union[nn.Linear, nn.Conv2d], rank, dropout=0.1, scale=1.0):
        ...
        if isinstance(host, nn.Linear):
            ...
            self.register_buffer('scale', torch.tensor(scale))
            self.lora_hook_handle = host.register_forward_hook(self.lora_forward_hook)
        elif isinstance(host, LayerNorm):
            self.host_type = 'layer_norm'
            self.lora_layer_norm = deepcopy(host)
            del self.lora_layer_norm.lora_block
            self.register_buffer('scale', torch.tensor(scale))
            self.lora_hook_handle = host.register_forward_hook(self.lora_forward_hook1)
        elif isinstance(host, BatchNorm2d):
            self.host_type = 'batch_norm'
            self.lora_batch_norm = deepcopy(host)
            self.register_buffer('scale', torch.tensor(scale))
            del self.lora_batch_norm.lora_block
            self.lora_hook_handle = host.register_forward_hook(self.lora_forward_hook2)
        else:
            raise NotImplementedError('lora support only Linear and Conv2d now.')

    def init_weights(self, sdv_init=False):
        host = self.host()
        if isinstance(host, nn.Linear) or isinstance(host, nn.Conv2d):
            if sdv_init:
            ...
            nn.init.zeros_(self.lora_layer_norm.weight)
            nn.init.zeros_(self.lora_layer_norm.bias)
        elif isinstance(host, BatchNorm2d):
            nn.init.zeros_(self.lora_batch_norm.weight)
            nn.init.zeros_(self.lora_batch_norm.bias)
    ...
    def lora_forward_hook1(self, host, fea_in, fea_out):
        return fea_out + self.lora_layer_norm(fea_in[0]) * self.scale
    def lora_forward_hook2(self, host, fea_in, fea_out):
        return fea_out + self.lora_batch_norm(fea_in[0]) * self.scale
    ...
    def collapse_to_host(self, alpha=None):
        ...
        elif self.host_type == 'layer_norm':
            host.weight = nn.Parameter(
                host.weight.data +
                alpha * (self.lora_layer_norm.weight.data).to(host.weight.device, dtype=host.weight.dtype))
            host.bias = nn.Parameter(
                host.bias.data +
                alpha * (self.lora_layer_norm.bias.data).to(host.bias.device, dtype=host.bias.dtype))
        elif self.host_type == 'batch_norm':
            host.weight = nn.Parameter(
                host.weight.data +
                alpha * (self.lora_batch_norm.weight.data).to(host.weight.device, dtype=host.weight.dtype))
            host.bias = nn.Parameter(
                host.bias.data +
                alpha * (self.lora_batch_norm.bias.data).to(host.bias.device, dtype=host.bias.dtype))
...
def warp_with_lora(model: nn.Module, rank, dropout=0.1, scale=1.0, sdv_init=False):
    ...
    for name in all_layers:
        if isinstance(named_modules[name], nn.Linear) or isinstance(named_modules[name], nn.Conv2d) or isinstance(named_modules[name], LayerNorm) or isinstance(named_modules[name], BatchNorm2d):
